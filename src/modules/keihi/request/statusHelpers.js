...
function parseKeihiLogIdFromFooter(embed) {
  const footerText = getFooterText(embed);
  if (!footerText) return null;
  return footerText.startsWith('LogID: ') ? footerText.slice('LogID: '.length).trim() : null;
}

function parseAdminLogIdFromKeihiLogContent(content) {
  const m = String(content || '').match(/AdminLogID:\s*(\d+)/);
  return m ? m[1] : null;
}
